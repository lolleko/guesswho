...
function GM:HandlePlayerJumping( ply, velocity )
    ...
    if ply.m_bJumping then
        ...
        if ply.m_bJumping then
            if ply:Team() == TEAM_HIDING then ply.CalcIdeal = ACT_JUMP else ply.CalcIdeal = ACT_MP_JUMP end
            return true
        end
    end
...
function GM:HandlePlayerDucking( ply, velocity )
    ...
    if ( velocity:Length2D() > 0.5 ) then
        if ply:Team() == TEAM_HIDING then ply.CalcIdeal = ACT_BUSY_SIT_GROUND else ply.CalcIdeal = ACT_MP_CROUCHWALK end
    else
        if ply:Team() == TEAM_HIDING then ply.CalcIdeal = ACT_BUSY_SIT_GROUND else ply.CalcIdeal = ACT_MP_CROUCH_IDLE end
    end
...
function GM:HandlePlayerVaulting( ply, velocity )
    ...
    if ply:Team() == TEAM_HIDING then ply.CalcIdeal = ACT_SWIM else ply.CalcIdeal = ACT_MP_SWIM end
...
function GM:CalcMainActivity( ply, velocity )
    ...
    if ply:Team() == TEAM_HIDING then ply.CalcIdeal = ACT_IDLE else ply.CalcIdeal = ACT_MP_STAND_IDLE end
    ...
    if ( self:HandlePlayerNoClipping( ply, velocity ) ||
        ...
        if ( len2d > 150 ) then
            if ply:Team() == TEAM_HIDING then ply.CalcIdeal = ACT_RUN else ply.CalcIdeal = ACT_MP_RUN end
        elseif ( len2d > 0.5 ) then
            if ply:Team() == TEAM_HIDING then ply.CalcIdeal = ACT_WALK else ply.CalcIdeal = ACT_MP_WALK end
        end
...
function GM:KeyRelease( ply, key)
    if key == IN_DUCK and ply:Team() == TEAM_HIDING then
        ply:SetSpeed(200)
        ply:SetWalkSpeed(100)
        ply:AnimRestartGesture(GESTURE_SLOT_CUSTOM, ACT_BUSY_SIT_GROUND_EXIT, true) -- true = autokill
        ply:AnimSetGestureWeight(GESTURE_SLOT_CUSTOM, 1)
    end
end

function GM:KeyPress( ply, key)
    if key == IN_USE and ply:Team() == TEAM_HIDING then
        ply:SetSpeed(50)
        PrintTable(ply:GetSequenceList())
        ply:AddVCDSequenceToGestureSlot(GESTURE_SLOT_CUSTOM, ply:LookupSequence("Lying_Down"),0, true) -- true = autokill
        ply:AnimSetGestureWeight(GESTURE_SLOT_CUSTOM, 1)
    end
end
*/